...
export function CatalogPreviewCard() {
  ...
  return (
    <Link
      to="/catalog"
      className="menu-tile group flex h-full flex-col gap-4 bg-[radial-gradient(circle_at_top_left,_rgba(255,218,120,0.28),_transparent_34%),linear-gradient(180deg,_rgba(255,255,255,0.94),_rgba(255,251,240,0.9))] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-amber-300 md:gap-5"
    >
      ...
      <div className="relative space-y-1">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          ...
          <div className="self-start rounded-full border border-amber-200/80 bg-white/80 px-3 py-1 text-[11px] font-bold uppercase tracking-[0.14em] text-amber-700 shadow-[0_8px_18px_rgba(217,119,6,0.12)]">
            {isLoading ? t('preview.statusLoading') : `${totalPokemon} ${t('preview.badge')}`}
          </div>
        </div>
      </div>

      <div className="relative grid gap-4 rounded-[1.55rem] border border-amber-100/80 bg-white/72 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.7)] md:rounded-[1.75rem]">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
          ...
          <div className="min-w-[6rem] self-start rounded-[1.25rem] border border-amber-100/80 bg-amber-50/70 px-4 py-3 text-left sm:text-right">
            ...
            <div className="md:hidden">
              <div className="grid grid-cols-3 gap-2">
                {Array.from({ length: 3 }).map((_, index) => (
                  <div key={index} className="rounded-[1.2rem] border border-white/80 bg-white/80 p-3">
                    <div className="h-16 rounded-[1rem] bg-slate-100" />
                    <div className="mt-3 h-3 w-10 rounded-full bg-slate-100" />
            ...
            <div className="md:hidden">
              <div className="grid grid-cols-3 gap-2">
                {featuredPokemon.map((pokemon) => (
                  <div
                    key={pokemon.id}
                    className="rounded-[1.2rem] border border-white/90 bg-white/84 p-3 shadow-[0_10px_24px_rgba(15,23,42,0.06)]"
                  >
                    <div className="flex h-16 items-center justify-center rounded-[1rem] bg-[radial-gradient(circle_at_top,_rgba(255,236,181,0.95),_rgba(255,255,255,0.92)_58%)]">
                      <img alt={pokemon.name} className="h-12 w-12 object-contain drop-shadow-[0_10px_12px_rgba(15,23,42,0.16)]" src={pokemon.sprite} />
                    </div>
                    <p className="mt-2 text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-400">{formatPokemonId(pokemon.id)}</p>
                    <p className="mt-1 break-words text-sm font-bold capitalize leading-5 text-ink">{pokemon.name}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="hidden gap-3 sm:grid-cols-[minmax(0,1fr)_auto] md:grid">
          <div className="rounded-[1.35rem] border border-white/90 bg-white/78 px-4 py-3">
            <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-400">{t('preview.scanLabel')}</p>
            <p className="mt-1 text-sm font-semibold text-ink">
              {firstPokemon ? `${formatPokemonId(firstPokemon.id)} ${firstPokemon.name}` : t('preview.statusEmpty')}
      ...
      <div className="mt-auto flex flex-col gap-3 rounded-[1.45rem] border border-slate-950/8 bg-white/88 px-4 py-3 shadow-[0_16px_28px_rgba(15,23,42,0.08)] sm:flex-row sm:items-center sm:justify-between">
        <div>
